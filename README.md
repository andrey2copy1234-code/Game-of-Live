...
Рекомендуется использовать -O3 -fopenmp для компиляции и компилятор g++ 15.1.0+. при этом проект намного быстрее при компиляции clang++ как я заметил. весь код написан под C++ 20.
### Android
...
set(CMAKE_CXX_FLAGS_DEBUG "${CMAKE_CXX_FLAGS_DEBUG} -O3 -fopenmp -static-openmp")
